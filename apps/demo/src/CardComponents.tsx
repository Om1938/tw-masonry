...
export function QuoteCard({ card }: { card: DemoCard }) {
    return (
        <div className="rounded-2xl bg-gray-900 border border-gray-800 p-6">
            <div className="text-4xl text-violet-500 font-serif leading-none mb-3">"</div>
            <p className="text-sm font-medium text-gray-200 leading-relaxed italic">
                {card.text}
            </p>
            <p className="mt-4 text-xs text-gray-500 font-semibold">— {card.author}</p>
        </div>
    );
}
    ...
    return (
        ...
        >
            ...
            <div className="relative z-10 flex-1 min-w-0">
                ...
                <p className="mt-1.5 text-sm text-white/75 leading-relaxed line-clamp-2">{card.excerpt}</p>
                <div className="mt-3 flex flex-wrap gap-1.5">
                    {card.tags?.map((t) => (
                        <span key={t} className="rounded-full bg-white/20 px-2.5 py-0.5 text-xs font-medium text-white/90">
                            {t}
                        </span>
                    ))}
                </div>
...
export function ImageCard({ card }: { card: DemoCard }) {
    return (
        ...
        >
            ...
            <span className="text-4xl mb-2 relative z-10 drop-shadow-lg">{card.emoji}</span>
            <p className="text-base font-bold text-white relative z-10 tracking-tight drop-shadow">{card.title}</p>
        </div>
    );
}
    ...
    return (
        ...
        >
            <div className="px-4 py-2.5 border-b border-violet-500/20 bg-violet-950/50 flex items-center gap-2 flex-shrink-0">
                ...
                <span className="text-xs font-mono font-semibold text-violet-300">resize demo</span>
                <span className="ml-auto text-[11px] text-gray-500 select-none">↕ drag corner</span>
            </div>
            <div className="p-4 flex-1">
                <p className="text-sm font-semibold text-white">{card.title}</p>
                <p className="text-xs text-gray-400 mt-2 leading-relaxed">{card.excerpt}</p>
                <div className="mt-4 inline-flex items-center gap-1.5 rounded-lg bg-violet-900/30 border border-violet-700/30 px-2.5 py-1">
                    <span className="text-[11px] text-violet-300 font-mono">observeResize: true</span>
                </div>
            </div>
        </div>
    );
...
export function Card({ card }: { card: DemoCard }) {
    switch (card.type) {
        case "gradient":
            return <GradientCard card={card} />;
        case "metric":
        ...
        case "resizable":
            return <ResizableCard card={card} />;
        case "wide":
            return <WideCard card={card} />;
    }
}
