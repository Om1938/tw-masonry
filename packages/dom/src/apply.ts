import type { LayoutResult, MasonryConfig } from "@tw-masonry/core";
import type { MeasuredItem } from "./measure.js";
...
export function clearItemPositionStyles(items: HTMLElement[]): void {
  items.forEach((item) => {
    item.style.position = "";
    item.style.left = "";
    item.style.top = "";
...
export function applyCssColumns(
...
export function applyJsLayout(
  ...
  const transition =
    config.transition?.enabled && !shouldReduceMotion()
